            ...
            if (((data[offset] & 0x80) != 0) && ((data[offset + 1] & 0x80) == 0)) {
                ...
                if ((data[offset] & 0xff) == 0xff) {
                    ...
                    if (_lookup.containsKey(value)) {
                        ...
                        if (D)
                            Log.w(getDriverName(), "Sending button event, button: " + e.getKeyCode() + ", direction: " + (e.getAction() == KeyEvent.ACTION_DOWN ? "DOWN" : "UP"));
                        ...
                        if (D)
                            Log.w(getDriverName(), "Unmatched button press: " + getHexString(data, offset, 2) + ", full block: " + getHexString(data, 0, data.length));
                    }
                ...
                if (D)
                    Log.w(getDriverName(), "Umatched byte #" + offset + " in: " + getHexString(data, 0, read));
                offset++;
                remaining--;
            }
        }
