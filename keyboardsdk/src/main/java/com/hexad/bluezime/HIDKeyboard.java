...
public class HIDKeyboard extends HIDReaderBase {
    ...
    public static final int HIDP_MOUSE_LEFT = 0x01;
    public static final int HIDP_MOUSE_RIGHT = 0x02;
    public static final int HIDP_MOUSE_MIDDLE = 0x04;
    ...
    private static final int[] MOUSE_KEY_MASKS = new int[]{
            HIDP_MOUSE_LEFT,
            HIDP_MOUSE_RIGHT,
            HIDP_MOUSE_MIDDLE
    };
    //List of key codes that correspond to the mouse above
    private static final int[] MOUSE_KEY_KEYS = new int[]{
            FutureKeyCodes.KEYCODE_VOLUME_UP,
            FutureKeyCodes.KEYCODE_VOLUME_DOWN,
            FutureKeyCodes.KEYCODE_VOLUME_MUTE
    };
    //List of modifier keys we can send key up/down events for
...
//        super.doConnect();
    ...
    public static int parseMouseKeys(int data) {
        int mourseKey = 0;
        if ((data & HIDP_MOUSE_LEFT) != 0)
            mourseKey |= HIDP_MOUSE_LEFT;
        if ((data & HIDP_MOUSE_RIGHT) != 0)
            mourseKey |= HIDP_MOUSE_RIGHT;
        if ((data & HIDP_MOUSE_MIDDLE) != 0)
            mourseKey |= HIDP_MOUSE_MIDDLE;
        return mourseKey;
    }
    ...
    // FIXME: 2017/12/3  偏移
    public void handleMouseMessage(byte[] data) throws Exception {
        if (D)
            Log.w(LOG_NAME, "Got mouse message, bytes: " + getHexString(data, 0, data.length));
        //此刻时间（毫秒）
        Log.e(LOG_NAME, "handleMouseMessage: " + System.currentTimeMillis());
        int x = NublerUtil.getShort(data, 4);
        int y = NublerUtil.getShort(data, 6);
        int x2 = NublerUtil.getShort2(data, 4);
        int y2 = NublerUtil.getShort2(data, 6);
        int w = data[8];
        //        Log.w(LOG_NAME, "x = " + x + ", y = " + y);
        Log.w(LOG_NAME, "mouse move enent: x2 = " + x2 + ", y2 = " + y2 + ", w = " + w);

//        if (x == 0 && y == 0 && w == 0) {
//            return;
//        }
        Log.e("time", "handleMouseMessage: " + System.currentTimeMillis());
        mouseBroadcast.putExtra(BluezService.EVENT_MOUSE_X, x2);
        mouseBroadcast.putExtra(BluezService.EVENT_MOUSE_Y, y2);
        mouseBroadcast.putExtra(BluezService.EVENT_MOUSE_W, w);
        m_context.sendBroadcast(mouseBroadcast);
    }

    @Override
    public void handleHIDMessage(byte hidType, byte reportId, byte[] data) throws Exception {
        if (reportId == 0x01) {
            if (data.length < 5) {
                ...
                int scanMouseKeys = ((int) data[0]) & 0xff;
                int mouseKeys = parseMouseKeys(scanMouseKeys);
                //Figure out if any mouse keys (LEFT, RIGHT, MIDDLE) have changed state,
                // and send an appropriate key event
                for (int i = 0; i < MOUSE_KEY_MASKS.length; i++) {
                    if ((m_lastMouseKeys & MOUSE_KEY_MASKS[i]) != (mouseKeys & MOUSE_KEY_MASKS[i])) {
                        keypressBroadcast.putExtra(BluezService.EVENT_KEYPRESS_ACTION, (mouseKeys & MOUSE_KEY_MASKS[i]) == 0 ? KeyEvent.ACTION_UP : KeyEvent.ACTION_DOWN);
                        keypressBroadcast.putExtra(BluezService.EVENT_KEYPRESS_KEY, MOUSE_KEY_KEYS[i]);
                        ...
                        String act = (mouseKeys & MOUSE_KEY_MASKS[i]) == 0 ? "up" : "down";
                        Log.w(LOG_NAME, "mouse keyevent: " + "action = " + act + ", keyCode = " + MOUSE_KEY_KEYS[i]);
                    }
                }

                int scanmodifiers = ((int) data[1]) & 0xff;
                ...
                for (int i = 0; i < META_KEY_MASKS.length; i++) {
                    if ((m_lastModifiers & META_KEY_MASKS[i]) != (modifiers & META_KEY_MASKS[i])) {
                        ...
                        keypressBroadcast.putExtra(BluezService.EVENT_KEYPRESS_ANALOG_EMULATED, false);
                        m_context.sendBroadcast(keypressBroadcast);
                        String act = (modifiers & META_KEY_MASKS[i]) == 0 ? "up" : "down";
                        Log.w(LOG_NAME, "modifiers keyevent: " + "action = " + act + ", keyCode = " + META_KEY_KEYS[i]);
                    }
                }
                ...
                for (int i = 0; i < pressedcount; i++) {
                    ...
                    if (pressed == -1) {
                        ...
                        keypressBroadcast.putExtra(BluezService.EVENT_KEYPRESS_ANALOG_EMULATED, false);
                        m_context.sendBroadcast(keypressBroadcast);
                        Log.w(LOG_NAME, "normal keyevent: " + "action = down" + ", keyCode = " + keycode);
                    }
                }
                ...
                for (int i = 0; i < m_lastPressedCount; i++) {
                    ...
                    if (m_lastPressed[i] != 0) {
                        ...
                        keypressBroadcast.putExtra(BluezService.EVENT_KEYPRESS_ANALOG_EMULATED, false);
                        m_context.sendBroadcast(keypressBroadcast);
                        Log.w(LOG_NAME, "normal keyevent: " + "action = up" + ", keyCode = " + m_lastPressed[i]);
                    }
                }
