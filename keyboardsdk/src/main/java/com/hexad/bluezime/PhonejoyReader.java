...
public class PhonejoyReader extends BGP100Reader {
    ...
    protected void handleAnalogValue(int axis, int value) {
        ...
        if (axis >= 0 && axis < 4) {
            ...
            if (D)
                Log.d(getDriverName(), "Axis " + axis + " changed to " + normalized + " (" + axis_value + ")");
            ...
            if (pressedUp != mAnalogKeyStates[(axis * 2)]) {
                ...
                if (D)
                    Log.w(getDriverName(), "Sending button event, button: " + ANALOG_KEYS[(axis * 2)] + ", direction: " + (pressedUp ? "DOWN" : "UP"));
            ...
            if (pressedDown != mAnalogKeyStates[(axis * 2) + 1]) {
                ...
                if (D)
                    Log.w(getDriverName(), "Sending button event, button: " + ANALOG_KEYS[(axis * 2) + 1] + ", direction: " + (pressedDown ? "DOWN" : "UP"));
