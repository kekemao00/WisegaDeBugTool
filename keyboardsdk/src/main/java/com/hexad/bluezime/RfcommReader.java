...
public abstract class RfcommReader implements BluezDriverInterface {
    ...
    protected Intent mouseBroadcast = new Intent(BluezService.EVENT_MOUSE);
    ...
    protected void doConnect() throws Exception {
        try {
            ...
            if (D)
                Log.d(LOG_NAME, "Welcome message from controller was " + getHexString(header, 0, read));
    ...
    public void run() {
        ...
        while (m_isRunning) {
            try {
                if (D)
                    Log.e(LOG_NAME + getDriverName(), "Buffer before read(" + unparsed + "): " + getHexString(buffer, 0, buffer.length));

                read = m_input.read(buffer, unparsed, buffer.length - unparsed);
                errors = 0;

                if (D)
                    Log.e(LOG_NAME + getDriverName(), "Buffer after read(" + read + " + " + unparsed + "): " + getHexString(buffer, 0, buffer.length));
                ...
                if (unparsed >= buffer.length - 10) {
                    if (D)
                        Log.e(LOG_NAME + getDriverName(), "Dumping unparsed data: " + getHexString(buffer, 0, unparsed));
                ...
                if (unparsed > 0 && read != unparsed) {
                    if (D)
                        Log.e(LOG_NAME + getDriverName(), "Sliding array before: " + getHexString(buffer, 0, buffer.length));

                    if (D)
                        Log.e(LOG_NAME + getDriverName(), "Sliding array params, read: " + read + ", unparsed: " + unparsed);

                    System.arraycopy(buffer, read - unparsed, buffer, 0, unparsed);

                    if (D)
                        Log.e(LOG_NAME + getDriverName(), "Sliding array after: " + getHexString(buffer, 0, buffer.length));
                }
